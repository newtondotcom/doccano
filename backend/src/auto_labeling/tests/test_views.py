...
class TestTemplateList(CRUDMixin):
    def setUp(self):
        ...
        self.url = reverse(
            viewname="auto_labeling_templates", args=[self.project.item.id]
        )
...
class TestConfigParameter(CRUDMixin):
    def setUp(self):
        ...
        self.url = reverse(
            viewname="auto_labeling_parameter_testing", args=[self.project.item.id]
        )
    ...
    def test_called_with_proper_model(self, mock):
        ...
        expected = RequestModelFactory.create(
            self.data["model_name"], self.data["model_attrs"]
        )
...
class TestTemplateMapping(CRUDMixin):
    def setUp(self):
        ...
        self.url = reverse(
            viewname="auto_labeling_template_test", args=[self.project.item.id]
        )
...
class TestLabelMapping(CRUDMixin):
    def setUp(self):
        ...
        self.url = reverse(
            viewname="auto_labeling_mapping_test", args=[self.project.item.id]
        )
...
class TestConfigCreation(CRUDMixin):
    def setUp(self):
        ...
        self.url = reverse(
            viewname="auto_labeling_configs", args=[self.project.item.id]
        )
    ...
    def test_list_config(self):
        baker.make("AutoLabelingConfig", project=self.project.item)
        response = self.assert_fetch(self.project.admin, status.HTTP_200_OK)
        self.assertEqual(len(response.data), 1)
...
class TestAutomatedLabeling(CRUDMixin):
    def setUp(self):
        self.project = prepare_project(
            task=ProjectType.DOCUMENT_CLASSIFICATION, single_class_classification=False
        )
        self.example = make_doc(self.project.item)
        self.category_pos = baker.make(
            "CategoryType", project=self.project.item, text="POS"
        )
        self.category_neg = baker.make(
            "CategoryType", project=self.project.item, text="NEG"
        )
        self.loc = baker.make("SpanType", project=self.project.item, text="LOC")
    ...
    @patch(
        ...
        return_value=Categories([{"label": "POS"}]),
    )
    def test_category_labeling(self, mock):
        baker.make(
            "AutoLabelingConfig", task_type="Category", project=self.project.item
        )
    ...
    @patch(
        "auto_labeling.views.execute_pipeline",
        return_value=Categories([{"label": "NEUTRAL"}]),
    )
    ...
    def test_allow_multi_type_configs(self, mock):
        ...
        baker.make("AutoLabelingConfig", task_type="Span", project=self.project.item)
        self.assert_create(self.project.admin, status.HTTP_201_CREATED)
        self.assertEqual(Category.objects.count(), 1)
    ...
    def test_cannot_use_other_project_config(self, mock):
        baker.make("AutoLabelingConfig", task_type="Category")
        self.assert_create(self.project.admin, status.HTTP_201_CREATED)
        self.assertEqual(Category.objects.count(), 0)
...
class TestAutomatedTextLabeling(CRUDMixin):
    ...
    @patch(
        "auto_labeling.views.execute_pipeline",
        side_effect=[Texts([{"text": "foo"}]), Texts([{"text": "foo"}])],
    )
    def test_cannot_label_same_text(self, mock):
        baker.make("AutoLabelingConfig", task_type="Text", project=self.project.item)
        baker.make("AutoLabelingConfig", task_type="Text", project=self.project.item)
        self.assert_create(self.project.admin, status.HTTP_201_CREATED)
