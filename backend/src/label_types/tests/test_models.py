...
class TestLabel(TestCase):
    def test_deny_creating_same_text(self):
        label = baker.make("CategoryType")
        with self.assertRaises(IntegrityError):
            baker.make("CategoryType", project=label.project, text=label.text)

    def test_keys_uniqueness(self):
        label = baker.make("CategoryType", prefix_key="ctrl", suffix_key="a")
    ...
    def test_suffix_key_uniqueness(self):
        label = baker.make("CategoryType", prefix_key=None, suffix_key="a")
        with self.assertRaises(ValidationError):
            CategoryType(
                project=label.project,
                text="example",
    ...
    def test_can_add_label_suffix_key_with_prefix_key(self):
        project = baker.make("Project")
        label = CategoryType(
            project=project, text="example", prefix_key="ctrl", suffix_key="a"
        )
        try:
            label.full_clean()
        except ValidationError:
