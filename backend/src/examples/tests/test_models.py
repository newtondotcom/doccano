...
class TestExampleState(TestCase):
    def setUp(self):
        ...
        self.other = baker.make("Example", project=self.project.item)
    ...
    def test_done_confirmed_by_multiple_user(self):
        ...
        baker.make(
            "ExampleState", example=self.example, confirmed_by=self.project.approver
        )
        done = ExampleState.objects.count_done(self.examples)
        self.assertEqual(done, 1)
    ...
    def test_done_confirmed_by_different_example(self):
        ...
        baker.make(
            "ExampleState", example=self.other, confirmed_by=self.project.approver
        )
        done = ExampleState.objects.count_done(self.examples, self.project.admin)
        self.assertEqual(done, 1)

    def test_initial_user(self):
        progress = ExampleState.objects.measure_member_progress(
            self.examples, self.project.members
        )
        expected_progress = [
    ...
    def test_user_count_after_multiple_user_confirmation(self):
        ...
        baker.make("ExampleState", example=self.example, confirmed_by=user1)
        baker.make("ExampleState", example=self.example, confirmed_by=user2)
        progress = ExampleState.objects.measure_member_progress(
            self.examples, self.project.members
        )
        expected_progress = [
