...
class TestBoundingBox(TestCase):
    ...
    def test_cannot_create_label_if_x_is_less_than_zero(self):
        with self.assertRaises(IntegrityError):
            baker.make(
                "BoundingBox", example=self.example, x=-1, y=0, width=0, height=0
            )

    def test_cannot_create_label_if_y_is_less_than_zero(self):
        with self.assertRaises(IntegrityError):
            baker.make(
                "BoundingBox", example=self.example, x=0, y=-1, width=0, height=0
            )

    def test_cannot_create_label_if_width_is_less_than_zero(self):
        with self.assertRaises(IntegrityError):
            baker.make(
                "BoundingBox", example=self.example, x=0, y=0, width=-1, height=0
            )

    def test_cannot_create_label_if_height_is_less_than_zero(self):
        with self.assertRaises(IntegrityError):
            baker.make(
                "BoundingBox", example=self.example, x=0, y=0, width=0, height=-1
            )
