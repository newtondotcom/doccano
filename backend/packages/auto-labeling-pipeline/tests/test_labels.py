...
class TestClassificationLabels:
    ...
    def test_convert_label(self, example_classification_data):
        ...
        labels = labels.to_dict_list()
        expected = [{"label": "A"}, {"label": "B"}, {"label": "D"}]
        assert labels == expected
    ...
    def test_merge(self):
        ...
        actual = sorted(labels.to_dict_list(), key=lambda x: x["label"])
...
class TestSequenceLabels:
    def test_filter_by_name(self, example_sequence_data):
        ...
        labels = labels.to_dict_list()
        expected = [{"label": "A", "start_offset": 0, "end_offset": 1}]
        assert labels == expected
    ...
    def test_remove_overlap(self, example_sequence_overlap_data):
        ...
        assert len(labels.to_dict_list()) == 1
    ...
    def test_merge(self):
        ...
        actual = sorted(labels.to_dict_list(), key=lambda x: x["start_offset"])
        assert actual == expected
...
class TestSeq2seqLabels:
    ...
    def test_merge(self):
        ...
        actual = sorted(labels.to_dict_list(), key=lambda x: x["text"])
        assert actual == expected
