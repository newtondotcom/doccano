...
class TestClassificationLabels:
    @pytest.mark.parametrize(
        "label, labels, expected", [("A", {"A"}, True), ("A", {"B"}, False)]
    )
    ...
    @pytest.mark.parametrize(
        "label, other, expected", [("A", "A", True), ("A", "B", False)]
    )
...
class TestSeq2seqLabel:
    @pytest.mark.parametrize(
        "text, labels, expected", [("A", {"A"}, True), ("A", {"B"}, False)]
    )
    ...
    @pytest.mark.parametrize(
        "text, other, expected", [("A", "A", True), ("A", "B", False)]
    )
