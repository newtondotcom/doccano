...
class TestSpanLabeling(abc.ABC, TestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.label_type = baker.make("SpanType", project=cls.project.item)
...
class TestSpan(TestCase):
    def setUp(self):
        self.project = prepare_project(
            ProjectType.SEQUENCE_LABELING, allow_overlapping=False
        )
    ...
    def test_start_offset_is_not_negative(self):
        with self.assertRaises(ValidationError):
            baker.make("Span", start_offset=-1, end_offset=0)

    def test_end_offset_is_not_negative(self):
        with self.assertRaises(ValidationError):
            baker.make("Span", start_offset=-2, end_offset=-1)

    def test_start_offset_is_less_than_end_offset(self):
        with self.assertRaises(ValidationError):
            baker.make("Span", start_offset=0, end_offset=0)

    def test_unique_constraint(self):
        baker.make(
            "Span", example=self.example, start_offset=5, end_offset=10, user=self.user
        )
        baker.make(
            "Span", example=self.example, start_offset=0, end_offset=5, user=self.user
        )
        baker.make(
            "Span", example=self.example, start_offset=10, end_offset=15, user=self.user
        )
    ...
    def test_unique_constraint_if_overlapping_is_allowed(self):
        ...
        example = baker.make("Example", project=project.item)
        user = project.admin
        baker.make("Span", example=example, start_offset=5, end_offset=10, user=user)
        spans = [(5, 10), (5, 11), (4, 10), (6, 9), (9, 15), (0, 6)]
        for start_offset, end_offset in spans:
            baker.make(
                "Span",
    ...
    def test_update(self):
        span = baker.make("Span", example=self.example, start_offset=0, end_offset=5)
        span.end_offset = 6
        span.save()
...
class TestSpanWithoutCollaborativeMode(TestCase):
    def setUp(self):
        self.project = prepare_project(
            ProjectType.SEQUENCE_LABELING, False, allow_overlapping=False
        )
        self.example = baker.make("Example", project=self.project.item)
...
class TestSpanWithCollaborativeMode(TestCase):
    def test_deny_users_to_create_same_spans(self):
        project = prepare_project(
            ProjectType.SEQUENCE_LABELING, True, allow_overlapping=False
        )
        example = baker.make("Example", project=project.item)
        baker.make(
            "Span", example=example, start_offset=5, end_offset=10, user=project.admin
        )
        with self.assertRaises(ValidationError):
            baker.make(
                "Span",
                ...
                start_offset=5,
                end_offset=10,
                user=project.approver,
            )

    def test_allow_users_to_create_same_spans_if_overlapping_is_allowed(self):
        project = prepare_project(
            ProjectType.SEQUENCE_LABELING, True, allow_overlapping=True
        )
        example = baker.make("Example", project=project.item)
        baker.make(
            "Span", example=example, start_offset=5, end_offset=10, user=project.admin
...
class TestLabelDistribution(TestCase):
    ...
    def test_calc_label_distribution(self):
        label_a = baker.make("SpanType", text="labelA", project=self.project.item)
        label_b = baker.make("SpanType", text="labelB", project=self.project.item)
        baker.make(
            "Span",
            example=self.example,
            start_offset=5,
        ...
        expected = {
            user.username: {label.text: 0 for label in SpanType.objects.all()}
