    ...
    <v-card>
        ...
        <v-card-text>
            ...
            <div v-if="selected === 'JSONL(Relation)'">
                <p class="body-1">
                    For readability, the above format can be displayed as
                    follows:
                </p>
                <v-sheet
                    :dark="!$vuetify.theme.dark"
...
export default {
    ...
    computed: {
        ...
        properties() {
            const item = this.catalog.find(
                (item) => item.displayName === this.selected,
            );
            if (item) {
        ...
        textFields() {
            ...
            const textFields = asArray.filter(
    ...
    async created() {
        this.catalog = await this.$repositories.catalog.list(
    ...
    methods: {
        ...
        pollData() {
            this.polling = setInterval(async () => {
                if (this.taskId) {
                    const res = await this.$repositories.taskStatus.get(
                        this.taskId,
                    );
                    if (res.ready) {
                        this.taskId = null;
                        ...
                        if (this.errors.length === 0) {
                            this.$router.push(
                                `/projects/${this.$route.params.id}/dataset`,
                            );
                        }
                    }
                }
