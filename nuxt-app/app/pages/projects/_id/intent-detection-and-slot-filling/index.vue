...
export default {
    ...
    computed: {
        ...
        shortKeys() {
            return Object.fromEntries(
                this.categoryTypes.map((item) => [item.id, [item.suffixKey]]),
            );
        },
    ...
    async created() {
        ...
        this.categoryTypes = await this.$services.categoryType.list(
            this.projectId,
        );
    ...
    methods: {
        async listSpan(docId) {
            const spans = await this.$services.sequenceLabeling.list(
                this.projectId,
                docId,
            );
            this.spans = spans;
        ...
        async listCategory(id) {
            this.categories = await this.$repositories.category.list(
                this.projectId,
                id,
            );
        },
        ...
        async removeCategory(id) {
            await this.$repositories.category.delete(
                this.projectId,
                this.doc.id,
                id,
            );
        ...
        async clear() {
            await this.$repositories.category.clear(
                this.projectId,
                this.doc.id,
            );
            await this.$services.sequenceLabeling.clear(
                this.projectId,
                this.doc.id,
            );
            await this.listSpan(this.doc.id);
        ...
        async autoLabel(docId) {
            try {
                await this.$services.sequenceLabeling.autoLabel(
                    this.projectId,
                    docId,
                );
            } catch (e) {
