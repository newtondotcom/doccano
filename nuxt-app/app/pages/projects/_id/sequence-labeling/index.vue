            ...
            <v-card class="mt-4">
                <v-card-title>
                    ...
                    <v-btn icon @click="showLabelTypes = !showLabelTypes">
                        <v-icon>{{
                            showLabelTypes ? mdiChevronUp : mdiChevronDown
                        }}</v-icon>
                    </v-btn>
                </v-card-title>
                <v-expand-transition>
...
export default {
    ...
    async created() {
        ...
        this.relationTypes = await this.$services.relationType.list(
            this.projectId,
        );
    ...
    methods: {
        async maybeFetchSpanTypes(annotations) {
            ...
            if (annotations.some((item) => !labelIds.has(item.label))) {
                this.spanTypes = await this.$services.spanType.list(
                    this.projectId,
                );
            }
        },
        ...
        async list(docId) {
            ...
            const relations =
                await this.$services.sequenceLabeling.listRelations(
                    this.projectId,
                    docId,
                );
