...
export default {
    ...
    computed: {
        ...
        regionList() {
            return this.annotations.map((annotation) => {
                return {
                    id: annotation.uuid,
                    category: this.labels.find(
                        (label) => annotation.label === label.id,
                    ).text,
                    color: this.labels.find(
                        (label) => annotation.label === label.id,
                    ).backgroundColor,
                    visibility:
                        annotation.uuid in this.visibilities
                            ? this.visibilities[annotation.uuid]
                            : true,
                };
            });
        },
        ...
        filteredRegions() {
            return this.annotations
                .filter(
                    (annotation) =>
                        this.visibilities[annotation.uuid] !== false,
                )
                .map((a) => {
                    return {
                        ...a,
    ...
    async created() {
        ...
        this.progress = await this.$repositories.metrics.fetchMyProgress(
            this.projectId,
        );
    },
    ...
    methods: {
        async list(imageId) {
            this.annotations = await this.$services.segmentation.list(
                this.projectId,
                imageId,
            );
        },
        ...
        async remove(id) {
            ...
            await this.$services.segmentation.delete(
                this.projectId,
                this.image.id,
                segmentation.id,
            );
        ...
        async update(region) {
            const segmentation = this.annotations.find(
                (a) => a.uuid === region.id,
            );
            await this.$services.segmentation.update(
                this.projectId,
        ...
        async clear() {
            await this.$services.segmentation.clear(
                this.projectId,
                this.image.id,
            );
            await this.list(this.image.id);
        ...
        async autoLabel(imageId) {
            try {
                await this.$services.segmentation.autoLabel(
                    this.projectId,
                    imageId,
                );
            } catch (e) {
        ...
        selectRegion(regionId) {
            if (regionId) {
                this.selectedRegion = this.annotations.find(
                    (r) => r.uuid === regionId,
                );
                this.selectedLabelIndex = this.labels.findIndex(
                    (l) => l.id === this.selectedRegion.label,
                );
            } else {
