...
export default Vue.extend({
    ...
    methods: {
        ...
        toSelected(item: ExampleDTO) {
            const assigneeIds = item.assignments.map(
                (assignment) => assignment.assignee_id,
            );
            return this.members.filter((member) =>
                assigneeIds.includes(member.user),
            );
        },

        onAssignOrUnassign(item: ExampleDTO, newAssignees: MemberItem[]) {
            const newAssigneeIds = newAssignees.map(
                (assignee) => assignee.user,
            );
            const oldAssigneeIds = item.assignments.map(
                (assignment) => assignment.assignee_id,
            );
