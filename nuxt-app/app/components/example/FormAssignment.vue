            ...
            <v-container fluid>
                <v-row>
                    <v-card-title class="pb-0 pl-3"
                        >Select assignment strategy</v-card-title
                    >
                    <v-col cols="12">
                        <v-select
                        ...
                        The project managers have access to all examples,
                        regardless of whether they are assigned or not.
                    </v-col>
                </v-row>
                <v-row>
                    <v-card-title class="pb-0 pl-3"
                        >Allocate weights</v-card-title
                    >
                    <v-col
...
export default Vue.extend({
    ...
    async fetch() {
        ...
        this.workloadAllocation = this.members.map(() =>
            Math.round(100 / this.members.length),
        );
    },
    ...
    computed: {
        ...
        strategies() {
            return [
                ...
                },
                {
                    ...
                    description:
                        "Assign examples to members randomly without replacement.",
                },
            ];
        },
        ...
        validateWeight(): boolean {
            if (this.selectedStrategy === "sampling_without_replacement") {
                ...
                return (
                    this.workloadAllocation.reduce(
                        (acc, cur) => acc + cur,
                        0,
                    ) === 100
                );
