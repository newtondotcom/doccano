        ...
        <v-card>
            <v-card-text class="pa-0">
                <v-form>
                    ...
                    <p class="font-weight-regular body-1">
                        Before proceeding, you need to test the parameters
                        whether they can fetch API response. Please input sample
                        text and press the
            ...
            <v-card-actions class="pa-0">
                <v-spacer />
                <v-btn text class="text-capitalize" @click="$emit('prev')">
                    Prev
                </v-btn>
                <v-btn
                    v-show="!isPassed"
...
onMounted(async () => {
    project.value = await $services.project.findById(route.params.id);
});
</script>
