...
export default Vue.extend({
        ...
        return {
            ...
            rules: {
                ...
                counter: (
                    ...
                    (v && v.length <= 100) ||
                    this.$t("rules.labelNameRules").labelLessThan100Chars,
                nameDuplicated: (
                    v: string, // @ts-ignore
                ) =>
                    !this.isUsedName(v) ||
                    this.$t("rules.labelNameRules").duplicated,
                keyDuplicated: (
                    v: string, // @ts-ignore
                ) =>
                    !this.isUsedSuffixKey(v) ||
                    this.$t("rules.keyNameRules").duplicated,
                validColor: (v: string) =>
                    /^#[0-9A-F]{6}$/i.test(v) ||
                    "This string is NOT a valid hex color.",
            },
            mdiReload,
        };
    },
    ...
    computed: {
        availableSuffixKeys(): string[] {
            ...
            return allSuffixKeys.filter(
                (item) => !usedSuffixKeys.includes(item),
            );
        },
        ...
        selectedColorIndex(value) {
            if (value < this.predefinedColors.length) {
                this.$emit(
                    ...
                    this.predefinedColors[this.selectedColorIndex],
                );
            }
        },
    },
    ...
    methods: {
        isUsedName(text: string): boolean {
            return (
                this.items.filter(
                    (item) => item.id !== this.id && item.text === text,
                ).length > 0
            );
        },
        ...
        isUsedSuffixKey(key: string) {
            ...
            return (
                this.items.filter(
                    (item) => item.id !== this.id && item.suffixKey === key,
                ).length > 0
            );
        },
