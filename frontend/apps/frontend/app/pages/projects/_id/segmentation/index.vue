...
watch(selectedLabel, async (newLabel) => {
  if (newLabel !== undefined && !!selectedRegion.value) {
    ...
    await $services.segmentation.update(
      projectId.value,
      image.value.id,
...
async function list(imageId) {
  annotations.value = await $services.segmentation.list(projectId.value, imageId);
}

async function remove(id) {
  delete visibilities.value[id];
  const segmentation = annotations.value.find((a) => a.uuid === id);
  await $services.segmentation.delete(projectId.value, image.value.id, segmentation.id);
  await list(image.value.id);
}

async function add(region) {
  visibilities.value[region.id] = true;
  await $services.segmentation.create(
    projectId.value,
    image.value.id,
    region.id,
    region.label,
...
async function update(region) {
  const segmentation = annotations.value.find((a) => a.uuid === region.id);
  await $services.segmentation.update(projectId.value, image.value.id, segmentation.id, region);
  await list(image.value.id);
}
...
async function clear() {
  await $services.segmentation.clear(projectId.value, image.value.id);
  await list(image.value.id);
}

async function autoLabel(imageId) {
  try {
    await $services.segmentation.autoLabel(projectId.value, imageId);
  } catch (e) {
