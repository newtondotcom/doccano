...
describe("TextSelector", () => {
  ...
  it("allowOverlapping=false and graphemeMode=false", () => {
    ...
    expect(() => textSelector.getOffsets(entitiesIntersect, text)).toThrowError(RangeError);
  });
  ...
  it("getRange gets the same start/end offsets", () => {
    const allowOverlapping = true;
    const graphemeMode = true;
    const spy = jest.spyOn(TextSelector.prototype as any, "getRange").mockReturnValue([0, 0]);
    const textSelector = new TextSelector(allowOverlapping, graphemeMode);
    expect(() => textSelector.getOffsets(entitiesNotIntersect, text)).toThrowError(RangeError);
    spy.mockClear();
  });
});
